...
class ActivePlanProvider with ChangeNotifier {
  ...
  ActivePlanProvider({AuthProvider? authProvider})
  ...
  Future<void> _loadActivePlan() async {
    ...
    final activePlanId = prefs.getString(
      'active_plan_id_${_authProvider!.currentUser!.name}',
    );
    print(
      'ActivePlanProvider: _loadActivePlan - activePlanId salvo: $activePlanId',
    );
    ...
    print(
      'ActivePlanProvider: _loadActivePlan - Planos lidos do DB: ${_allPlans.length}',
    );
    for (var plan in _allPlans) {
      print(
        'ActivePlanProvider: _loadActivePlan - Plano ID no _allPlans: ${plan.id}',
      );
    }
    ...
    if (activePlanId != null) {
      try {
        ...
        print(
          'ActivePlanProvider: _loadActivePlan - Plano ativo encontrado: ${_activePlan?.name}',
        );
      } catch (e) {
        print(
          'ActivePlanProvider: _loadActivePlan - Erro ao encontrar plano ativo salvo: $e',
        );
      ...
      if (_activePlan != null) {
        print(
          'ActivePlanProvider: _loadActivePlan - Primeiro plano definido como ativo: ${_activePlan?.name}',
        );
      }
    }
    notifyListeners();
  ...
  Future<void> setActivePlan(String planId) async {
    if (_authProvider?.currentUser == null) return;
    print(
      'ActivePlanProvider: setActivePlan - Tentando definir plano ID: $planId',
    );
    final prefs = await SharedPreferences.getInstance();
    ...
    _allPlans = await _dbService.readAllPlans(_authProvider!.currentUser!.name);
    print(
      'ActivePlanProvider: setActivePlan - Planos lidos do DB: ${_allPlans.length}',
    );
    for (var plan in _allPlans) {
      print(
        'ActivePlanProvider: setActivePlan - Plano ID no _allPlans: ${plan.id}',
      );
    }
    ...
    try {
      ...
      print(
        'ActivePlanProvider: setActivePlan - Plano ativo definido: ${_activePlan?.name}',
      );
    } catch (e) {
      print(
        'ActivePlanProvider: setActivePlan - Erro ao encontrar plano com ID $planId: $e',
      );
