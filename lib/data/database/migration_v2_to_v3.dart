...
Future<void> migrateV2ToV3(Database db) async {
  await db.transaction((txn) async {
    ...
    // Verificar se a tabela study_sessions existe antes de renomear
    final List<Map<String, Object?>> existingTables = await txn.rawQuery(
    ...
    // Verificar se a tabela backup existe antes de tentar migrar
    final List<Map<String, Object?>> backupExists = await txn.rawQuery(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='study_sessions_backup'"
    );

    if (backupExists.isNotEmpty) {
      // Obter dados da tabela antiga
      final List<Map<String, dynamic>> oldSessions = await txn.rawQuery('''
      ...
      for (final Map<String, dynamic> session in oldSessions) {
        try {
          final String? topicosJson = session['topicos_json'] as String?;
          ...
          final String? areaName = _extractAreaFromJson(topicosJson);
          final String? topicName = _extractTopicFromJson(topicosJson);
          ...
          // Buscar ID da área
          final List<Map<String, dynamic>> areaResult = await txn.rawQuery('''
            SELECT id FROM subject_areas WHERE name = ?
          ''', [areaName]);

          final int areaId = areaResult.isNotEmpty ? (areaResult.first['id'] as int) : 0;
          ...
          // Buscar ID do tópico
          final List<Map<String, dynamic>> topicResult = await txn.rawQuery('''
            SELECT id FROM topics WHERE subject_area_id = ? AND name = ?
          ''', [areaId, topicName]);

          final int topicId = topicResult.isNotEmpty ? (topicResult.first['id'] as int) : 0;
      ...
      final List<Map<String, dynamic>> countOld = await txn.rawQuery('SELECT COUNT(*) as c FROM study_sessions_backup');
      final List<Map<String, dynamic>> countNew = await txn.rawQuery('SELECT COUNT(*) as c FROM study_sessions');
