...
class _StatsScreenState extends State<StatsScreen> {
        ...
        return Scaffold(
          body: Consumer2<HistoryProvider, AllSubjectsProvider>(
            builder: (context, historyProvider, allSubjectsProvider, child) {
              ...
              final filteredRecords = historyProvider.allStudyRecords.where((
                record,
              ) {
                // Date filter
                if (filterProvider.statsStartDate != null &&
                    DateTime.parse(
                      record.date,
                    ).isBefore(filterProvider.statsStartDate!)) {
                  return false;
                }
                if (filterProvider.statsEndDate != null &&
                    DateTime.parse(
                      record.date,
                    ).isAfter(filterProvider.statsEndDate!)) {
                  return false;
                }

                // Duration filter
                if (filterProvider.statsMinDuration != null &&
                    record.study_time <
                        filterProvider.statsMinDuration! * 60000) {
                  // convert minutes to ms
                  return false;
                }
                if (filterProvider.statsMaxDuration != null &&
                    record.study_time >
                        filterProvider.statsMaxDuration! * 60000) {
                  return false;
                }

                // Performance filter (agregando de topicsProgress)
                final aggregatedProgress =
                    AggregatedTopicProgress.fromStudyRecord(record);
                if (filterProvider.statsMinPerformance != null &&
                    aggregatedProgress.performance <
                        filterProvider.statsMinPerformance!) {
                  return false;
                }
                if (filterProvider.statsMaxPerformance != null &&
                    aggregatedProgress.performance >
                        filterProvider.statsMaxPerformance!) {
                  return false;
                }

                // Category filter
                if (filterProvider.statsSelectedCategories.isNotEmpty &&
                    !filterProvider.statsSelectedCategories.contains(
                      record.category,
                    )) {
                  return false;
                }
                ...
                if (filterProvider.statsSelectedSubjects.isNotEmpty) {
                  ...
                  if (subject.id.isEmpty ||
                      !filterProvider.statsSelectedSubjects.contains(
                        subject.subject,
                      )) {
                    return false;
                  }
                ...
                if (filterProvider.statsSelectedTopics.isNotEmpty) {
                  ...
                  for (var tp in record.topicsProgress) {
                    if (filterProvider.statsSelectedTopics.contains(
                      tp.topicText,
                    )) {
                      topicMatch = true;
                      break;
              ...
              // Calculations
              final int totalStudyTime = records.fold(
                0,
                (sum, record) => sum + record.study_time,
              );
              final int totalPagesRead = records.fold(0, (sum, record) {
                return sum +
                    record.topicsProgress.fold(0, (tpSum, tp) {
                      return tpSum +
                          tp.pages.fold(0, (pageSum, page) {
              ...
              final int totalVideoTime = records.fold(0, (sum, record) {
                return sum +
                    record.topicsProgress.fold(0, (tpSum, tp) {
                      return tpSum +
                          tp.videos.fold(0, (videoSum, video) {
              ...
              final uniqueStudyDays = records
                  .map((r) => r.date.split('T')[0])
                  .toSet()
                  .length;
              final firstRecordDate = records.isNotEmpty
                  ? DateTime.parse(records.map((r) => r.date).min)
                  : DateTime.now();
              final totalDaysSinceFirstRecord =
                  DateTime.now().difference(firstRecordDate).inDays + 1;
              final failedStudyDays =
                  totalDaysSinceFirstRecord - uniqueStudyDays;
              final studyConsistencyPercentage = totalDaysSinceFirstRecord > 0
                  ? (uniqueStudyDays / totalDaysSinceFirstRecord) * 100
                  : 0.0;
              ...
              final int totalCorrectQuestions = records.fold(0, (sum, record) {
                return sum +
                    record.topicsProgress.fold(
                      0,
                      (tpSum, tp) => tpSum + (tp.questions['correct'] ?? 0),
                    );
              });
              ...
              for (var record in records) {
                ...
                dailyStats.update(
                  ...
                  ifAbsent: () => {'correct': correct, 'incorrect': incorrect},
                );
              }
              ...
              for (var record in records) {
                final date = DateTime.parse(record.date.split('T')[0]);
                final hours =
                    record.study_time / 3600000; // Convert ms to hours
              ...
              for (var subject in subjects) {
                final subjectRecords = records.where(
                  (r) => r.subject_id == subject.id,
                );

                final totalHours = subjectRecords.fold(
                  0.0,
                  (sum, record) => sum + (record.study_time / 3600000),
                );
              ...
              final Map<Subject, SubjectPerformanceData>
              ...
              for (var computedSubject in computedSubjects) {
                ...
                if (totalQuestions > 0) {
                  subjectPerformanceData[computedSubject.originalSubject] =
                      SubjectPerformanceData(
              ...
              final topicPerformanceData = _buildTopicPerformanceData(
                computedSubjects,
              );
              ...
              return SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildSubjectHoursAndCategoryChart(
                      context,
                      subjectHours,
                      categoryHours,
                    ),
  ...
  List<HierarchicalPerformanceNode> _buildTopicPerformanceData(
    List<_ComputedSubject> computedSubjects,
  ) {
      ...
      if (totalQuestions > 0) {
        subjectNodes.add(
          HierarchicalPerformanceNode(
            ...
            percentualAcerto: totalQuestions > 0
            ...
            children: [
              ...
              Expanded(
                child: _buildSummaryCard(
                  ...
                  subtitle1:
                      '${formatTime(uniqueStudyDays > 0 ? (totalStudyTime / uniqueStudyDays).round() : 0)} por dia estudado (média)',
            ...
            children: [
              Expanded(
                child: _buildSummaryCard(
                  ...
                  subtitle1:
                      '${(totalPagesRead / (totalStudyTime > 0 ? totalStudyTime / 3600000 : 1)).toStringAsFixed(1)} páginas/hora',
                ),
              ),
          ...
          children: [
            Expanded(
              child: _buildSummaryCard(
                ...
                subtitle1:
                    '$uniqueStudyDays dias estudados de $totalDaysSinceFirstRecord dias',
            ...
            Expanded(
              child: _buildSummaryCard(
                ...
                subtitle1:
                    '${overallStats.completed} tópicos concluídos de ${overallStats.total}',

                subtitle2:
                    '(${overallStats.total - overallStats.completed} tópicos pendentes)',
              ),
            ),
          ],
        ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (subtitle2 != null)
              Text(subtitle2, style: TextStyle(color: Colors.grey[600])),
          ],
        ),
      ),
    );
  ...
  Widget _buildSubjectHoursAndCategoryChart(
    BuildContext context,
    Map<Subject, double> subjectHours,
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                    ...
                    Row(
                      children: [
                        IconButton(
                          ...
                          onPressed: () =>
                              setState(() => _subjectSortOrder = 'desc'),

                          color: _subjectSortOrder == 'desc'
                              ? Colors.teal
                              : Colors.grey,
                        ),
                        ...
                        IconButton(
                          ...
                          onPressed: () =>
                              setState(() => _subjectSortOrder = 'asc'),

                          color: _subjectSortOrder == 'asc'
                              ? Colors.teal
                              : Colors.grey,
                        ),
                        ...
                        IconButton(
                          ...
                          onPressed: () =>
                              setState(() => _subjectSortOrder = 'alpha'),

                          color: _subjectSortOrder == 'alpha'
                              ? Colors.teal
                              : Colors.grey,
                        ),
                      ],
                ...
                DisciplinasHorasChart(
                  subjectHours: subjectHours,
                  sortOrder: _subjectSortOrder,
                ),
              ],
            ),
          ),
