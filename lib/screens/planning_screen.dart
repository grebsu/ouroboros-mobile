...
class _PlanningScreenState extends State<PlanningScreen> {
  ...
  final TextEditingController _durationToAddController =
  ...
  void _onStartStudy(BuildContext context, StudySession session) async {
    ...
    final topic = subject != null
        ? _findTopicByText(subject.topics, session.subject)
        : null;
  ...
  void _onDeleteSession(String sessionId) {
    ...
    final updatedCycle = planningProvider.studyCycle!
  ...
  void _onDuplicateSession(StudySession session) {
    ...
    final updatedCycle = List<StudySession>.from(planningProvider.studyCycle!)
      ..add(newSession);
    planningProvider.setStudyCycle(updatedCycle);
  }

  void _onReorderSessions(
    List<StudySession> displayedList,
    int oldIndex,
    int newIndex,
  ) {
    final planningProvider = Provider.of<PlanningProvider>(
    ...
    final currentFullCycle = List<StudySession>.from(
      planningProvider.studyCycle!,
    );

    // 1. Perform the reorder operation on a mutable copy of the displayedList.
    final List<StudySession> reorderedDisplayedList = List<StudySession>.from(
      displayedList,
    );
    if (oldIndex < newIndex) {
      newIndex -=
          1; // Adjust index for ReorderableListView when moving downwards
    }
  ...
  Widget _buildPlanningDashboard(
    BuildContext context,
    PlanningProvider planningProvider,
  ) {
    final totalCycleDuration = planningProvider.studyCycle!.fold<int>(
      0,
          ...
          return <Widget>[
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Text(
                                    planningProvider.completedCycles.toString(),
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 10),
                                  Text(
                                    '${_formatDuration(planningProvider.currentProgressMinutes)} / ${_formatDuration(totalCycleDuration)}',
                            ...
                            ElevatedButton(
                              onPressed: () {
                                if (_subjectToAdd != null &&
                                    _durationToAddController.text.isNotEmpty) {
                                  ...
                                  final newSession = StudySession(
                                    ...
                                    duration: int.parse(
                                      _durationToAddController.text,
                                    ),
                                    color: _subjectToAdd!.color,
                                  );
                                  final updatedCycle = List<StudySession>.from(
                                    planningProvider.studyCycle!,
                                  )..add(newSession);
                                  planningProvider.setStudyCycle(updatedCycle);
                                  setState(() {
            ...
            SliverAppBar(
              ...
              automaticallyImplyLeading:
                  false, // Remove back button from SliverAppBar
              backgroundColor: Theme.of(
                context,
              ).scaffoldBackgroundColor, // Usa a cor de fundo do Scaffold
              title: TabBar(
                labelColor: Colors.teal == Brightness.dark
                    ? Colors.teal
                    : Colors.teal,
                unselectedLabelColor:
                    ...
                    ? Colors.grey[400]
                    : Colors.grey,
                indicatorColor: Colors.teal == Brightness.dark
                    ? Colors.teal
                    : Colors.teal,
                tabs: const [
          ...
          children: [
            ...
            Consumer<ActivePlanProvider>(
              builder: (context, activePlanProvider, child) {
                final pendingSessions = planningProvider.studyCycle!
                    .where(
                      (s) =>
                          (planningProvider.sessionProgressMap[s.id] ?? 0) <
                          s.duration,
                    )
                    .toList();
                return Column(
                    ...
                    Expanded(
                      child: StudySessionList(
                        ...
                        onStartStudy: (session) =>
                            _onStartStudy(context, session),
                        onRegisterStudy: (session) =>
                            _onRegisterStudy(context, session),
                        isEditMode: widget.isEditMode,
                        onDeleteSession: _onDeleteSession,
                        onDuplicateSession: _onDuplicateSession,
                        onReorder: (oldIdx, newIdx) =>
                            _onReorderSessions(pendingSessions, oldIdx, newIdx),
            ...
            Consumer<ActivePlanProvider>(
              builder: (context, activePlanProvider, child) {
                final completedSessions = planningProvider.studyCycle!
                    .where(
                      (s) =>
                          (planningProvider.sessionProgressMap[s.id] ?? 0) >=
                          s.duration,
                  ...
                  children: [
                    ...
                    Expanded(
                      child: StudySessionList(
                        ...
                        onDeleteSession: _onDeleteSession,
                        onDuplicateSession: _onDuplicateSession,
                        onReorder: (oldIdx, newIdx) => _onReorderSessions(
                          completedSessions,
                          oldIdx,
                          newIdx,
                        ),
                        ...
                        sessionProgressMap: planningProvider.sessionProgressMap,
                        extraStudyTimeBySubjectId: planningProvider
                            .extraStudyTimeBySubjectId, // Passando os dados aqui
                      ),
                    ),
                  ],
                );
