...
class DisciplinasHorasChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (sortOrder == 'desc') {
      sortedSubjects.sort(
        (a, b) => subjectHours[b]!.compareTo(subjectHours[a]!),
      );
    } else if (sortOrder == 'asc') {
      sortedSubjects.sort(
        (a, b) => subjectHours[a]!.compareTo(subjectHours[b]!),
      );
    } else {
      // alpha
    ...
    return SizedBox(
      height: 400,
      width: double
          .infinity, // Ajustar a largura para ocupar todo o espaço disponível
      child: RotatedBox(
        quarterTurns: 1,
        child: BarChart(
          BarChartData(
            ...
            barGroups: barGroups,
            minY: 0,
            maxY: subjectHours.isNotEmpty
                ? subjectHours.values.reduce((a, b) => a > b ? a : b) * 1.2
                : 1.0, // Ajusta o maxY para o maior valor + 20%
            titlesData: FlTitlesData(
              ...
              leftTitles: AxisTitles(
                // This is effectively the Y-axis for hours in the rotated view
              ...
              bottomTitles: AxisTitles(
                // This is effectively the X-axis for subjects in the rotated view
                sideTitles: SideTitles(
                  showTitles: true,
                  ...
                  getTitlesWidget: (value, meta) {
                    if (value.toInt() >= 0 &&
                      ...
                      return SideTitleWidget(
                        meta: meta,
                        angle:
                            -0.785, // Aproximadamente -45 graus em radianos para rotação diagonal
            ...
            gridData: FlGridData(
              show: true,
              drawVerticalLine:
                  false, // Vertical lines in original orientation, so horizontal in rotated
              getDrawingHorizontalLine: (value) {
                // Horizontal lines in original orientation, so vertical in rotated
                return const FlLine(
